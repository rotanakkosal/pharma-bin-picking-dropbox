...
// inference service). Returns a list of { path, overlay? } entries with
// overlay-only files folded into their source row.
function pairOverlays(files) {
...
function renderEntry(entry, fileUrl, sessionName) {
    const isImage = /\.(png|jpe?g)$/i.test(entry.path);
...
function renderFileRow(node, depth) {
    ...
    // thumbnails, both clickable to open in the lightbox.
    let previewHtml = '';
...
function renderDetailPanel() {
    ...
    if (!detail) {
        ...
        bodyHtml = `<div class="detail-body">${detail.sessions.map(s => {
            ...
            const { displayed } = pairOverlays(s.files);
            displayed.sort((a, b) =>
